...
```"""
    ...
    def test_routing_repair_is_narrow_and_restart_aware(self) -> None:
        ...
        self.assertIn("missing Fable launcher", SKILL)
        self.assertIn("saved pre-setup values were absent", SKILL)
        self.assertIn("leaves the original restore snapshot", SKILL)
        self.assertIn("preserves a concurrent config or saved-state edit", SKILL)
        self.assertIn("fully quit and reopen\nCodex", SKILL)
